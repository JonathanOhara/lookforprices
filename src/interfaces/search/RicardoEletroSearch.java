...
public class RicardoEletroSearch implements Search{
	...
	public List<Product> search(Shop shop, String productName, Filter filter) {
		...
		try{
			...
			Elements els = document.select(".box-resultados").select(".box-vitrine");
	...
	private List<Product> readEachProduct(Shop shop, String productName, Elements els, Filter filter) throws IOException {
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				previewName = productContainer.select(".nome-produto-vertical a").text();
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select("#ProdutoDetalhesPrecoComprarAgoraPrecoDePreco").size() > 0 ? document.select("#ProdutoDetalhesPrecoComprarAgoraPrecoDePreco").text().trim(): Keys.INDISPONIVEL;
