...
public class NetShoesSearch implements Search{
	...
	public List<Product> search(Shop shop, String productName, Filter filter) {
		...
		try{
			...
			Elements els = document.select(".single-product");
	...
	private List<Product> readEachProduct(Shop shop, String productName, Elements els, Filter filter) throws IOException {
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				previewName = productContainer.select(".name").text();
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select(".new-price-holder").size() > 0 ? document.select(".new-price-holder .new-price").text().trim(): Keys.INDISPONIVEL;
