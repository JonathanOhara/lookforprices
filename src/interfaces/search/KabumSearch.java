...
public class KabumSearch implements Search{
	...
	public List<Product> search(Shop shop, String productName, Filter filter) {
		...
		try{
			...
			Elements els = document.select(".listagem-box");
	...
	private List<Product> readEachProduct(Shop shop, String productName, Elements els, Filter filter) throws IOException {
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				previewName = productContainer.select(".H-titulo a").text();
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select(".preco_desconto").first().select("strong").first().text().toString().trim();
					gameCompleteName = document.select(".titulo_det").first().text();
