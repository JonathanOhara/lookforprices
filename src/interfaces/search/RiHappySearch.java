...
public class RiHappySearch implements Search{
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select(".skuBestPrice").size() > 0 ? document.select(".skuBestPrice").text().trim(): Keys.INDISPONIVEL;
	
					gameCompleteName = document.select(".fn").
							first().text();
