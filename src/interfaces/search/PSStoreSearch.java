...
public class PSStoreSearch implements Search{
	...
	private List<Product> readEachProduct(Shop shop, String productName, Elements els, Filter filter) throws IOException {
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				previewName = productContainer.select(".cellTitle").text();
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select(".price").size() > 0 ? document.select(".price").first().nextElementSibling().html().toString().trim(): Keys.INDISPONIVEL;
	
					if( document.select(".productTitle").size() > 0 ){
						gameCompleteName = document.select(".productTitle").first().text();
