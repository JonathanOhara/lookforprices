...
public class FastGamesSearch implements Search{
		...
		if( els.size() > 0 ){
			...
			for( Element element : els ){
				...
				if( filter.filter(previewName, productName) ){
					...
					price = document.select(".produto-preco .preco-avista").size() > 0 ? document.select(".produto-preco .preco-avista").text().trim(): Keys.INDISPONIVEL;
	
					gameCompleteName = document.select("h3.NomeProduto").text();
